...
## Snow data:
dat <- read_csv2(
    "~/Downloads/metobs_snowDepth_all_sites.csv",
    col_types = cols(
...
## Files downloaded frm SMHI
files <- list.files("~/Downloads/smhi_snow_data")
ids <- files %>% str_split("_") %>% map(function(x) x[4]) %>% unlist()
out <- list()
...
read_files <- function(x,y){
    ...
    # y is the ids
    obs <- read_delim(
        file = paste("~/Downloads/smhi_snow_data", x, sep = "/"),
        delim = ";",
        skip = 10,
        col_types = cols(
            ...
            `Markens tillstånd` = col_double(),
            Kvalitet_1 = col_character(),
            X7 = col_logical(),
    ...
    obs <- obs %>%
        select(1:4) %>%
        rename(date = 1, hour_UTC = 2, snow_depth_mts = 3, quality = 4)

    obs <- obs  %>%
        filter (date > min(times$day))

    annotated <- times %>% select(-dates_round) %>%
        # I need to join here by date, not hour, otherwise we miss all values
        # All measurements were taken at 0600
        left_join(obs, by = c("day" = "date")) %>%
        select(-day, time_picture = time, hour_measurement = hour_UTC) %>%
        filter(!is.na(snow_depth_mts))
...
write_csv(out, "annotated_snow.csv")
